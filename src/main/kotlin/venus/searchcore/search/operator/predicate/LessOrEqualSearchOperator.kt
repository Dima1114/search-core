...
@Service
class LessOrEqualSearchOperator : SearchOperator("loe") {
    override fun <T> getPredicate(booleanBuilder: BooleanBuilder, path: Path<T>, values: MutableList<String>) {
        when (path.type) {
            in longs -> booleanBuilder.and((path as NumberPath<*>).loe(values[0].toLong()))
            in doubles -> booleanBuilder.and((path as NumberPath<*>).loe(values[0].toDouble()))
        }
    }
}
