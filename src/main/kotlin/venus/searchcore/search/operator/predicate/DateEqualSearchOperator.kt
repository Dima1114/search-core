...
@Service
class DateEqualSearchOperator : SearchOperator("deq") {
    @Suppress("UNCHECKED_CAST")
    override fun <T> getPredicate(booleanBuilder: BooleanBuilder, path: Path<T>, values: MutableList<String>) {
        when (path.type) {
            localDateClass ->
                booleanBuilder.and((path as DatePath<LocalDate>).eq(localDateParser.convert(values[0])))
            localDateTimeClass ->
                booleanBuilder.and((path as DateTimePath<LocalDateTime>).eq(localDateTimeParser.convert(values[0])))
        }
    }
}
